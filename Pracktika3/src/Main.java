...
public class Main {
    ...
    public static int countIntegerRoots(int[] coefficients) {
        // Проверяем, что массив coefficients содержит ровно 3 элемента (a, b, c).
        if (coefficients.length != 3) {
            throw new IllegalArgumentException("Массив коэффициентов должен содержать 3 элемента (a, b, c).");
        }

        // Извлекаем коэффициенты a, b и c из массива.
        ...
        // Проверяем значение дискриминанта для определения числа корней.
        if (discriminant > 0) {
            return 0;
    ...
    public static char commonVowel(String sentence) {
        ...
        // Создаём мапу для подсчета гласных и их количества
        ...
        // Проходим через записи в мапе
        for (Map.Entry<Character, Integer> entry : vowelCount.entrySet()) {
            // Проверяем, если количество вхождений текущей гласной больше, чем текущее максимальное количество.
            if (entry.getValue() > maxCount) {
                // Если да, то обновляем mostCommonVowel и maxCount для этой гласной.
