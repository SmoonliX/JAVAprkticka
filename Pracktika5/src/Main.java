...
public class Main {
    ...
    static String spiderVsFly(String spider, String fly) {
        // Извлечение координат паука
        int spiderRing = spider.charAt(1) - '0';
        char spiderRadial = spider.charAt(0);

        // Извлечение координат мухи
        int flyRing = fly.charAt(1) - '0';
        char flyRadial = fly.charAt(0);

        // Создание объекта StringBuilder для построения пути
        StringBuilder path = new StringBuilder();

        // Строим путь паука от внешнего кольца к центру
        for (int i = spiderRing; i >= 1; i--) {
            path.append(spiderRadial).append(i).append("-");
        }

        // Строим путь мухи от центра к внешнему кольцу
        for (int i = 1; i <= flyRing; i++) {
            path.append(flyRadial).append(i).append("-");
        }

        // Убираем последний лишний символ "-" и возвращаем построенный путь
